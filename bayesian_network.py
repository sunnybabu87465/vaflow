...
if __name__ == '__main__':
    ...
    clf = BNClassifier(learningMethod="TAN", discretizationStrategy="kmeans", discretizationNbBins=5,
                       significant_digit=0)
    ...
    try:
        shutil.rmtree("bayesian_network_result")
    except:
        print('Folder not deleted')
    os.mkdir("bayesian_network_result")
    ...
    disp.plot()
    plt.savefig('bayesian_network_result/BNClassifierConfusionMatrix.png', bbox_inches='tight')
    open("bayesian_network_result/bayesian_network.dot", mode='x', encoding=locale.getpreferredencoding()).write(
        gum.BayesNet(clf.bn).toDot())
    # s = Source.from_file("bayesian_network_result/bayesian_network.dot")
    # s.render('bayesian_network_result/bayesian_network', format='jpg', view=True)

    plt.clf()
    skplt.metrics.plot_roc(np.asarray(y_test.values, int), predict_prob)
    plt.savefig('bayesian_network_result/BNClassifierROCurve.png', bbox_inches='tight')
