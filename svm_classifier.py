...
if __name__ == '__main__':
    ...
    clf = svm.NuSVC(nu=0.7,probability=True,class_weight="balanced",decision_function_shape="ovr")
    ...
    try:
        shutil.rmtree("svm_classifier_result")
    except:
        print('Folder not deleted')
    os.mkdir("svm_classifier_result")
    ...
    disp.plot()
    plt.savefig('svm_classifier_result/ConfusionMatrix.png', bbox_inches='tight')

    plt.clf()
    skplt.metrics.plot_roc(np.asarray(y_test.values, int), predict_prob)
    plt.savefig('svm_classifier_result/ROCurve.png', bbox_inches='tight')
